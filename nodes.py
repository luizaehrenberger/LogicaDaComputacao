...
class Node(ABC):
    ...
    # Por padrão, nós que não geram código levantam explicitamente
...
class IntVal(Node):
    ...
    def generate(self, st: SymbolTable) -> None:
        Code.append(f"  mov eax, {int(self.value)}")
...
class StringVal(Node):
    ...
    def generate(self, st: SymbolTable) -> None:
        # Não geramos código para strings neste roteiro
...
class Print(Node):
    ...
    def generate(self, st: SymbolTable) -> None:
        # Apenas números/boolean (0/1). Strings não suportadas.
...
class Assignment(Node):
    ...
    def generate(self, st: SymbolTable) -> None:
        ...
        if var.shift is None:
            raise Exception("[CodeGen] Variável sem deslocamento de pilha")
        expr_node.generate(st)  # resultado em eax
        Code.append(f"  mov [ebp-{var.shift}], eax")

# ---------- Declaração ----------
class VarDec(Node):
    """
    value: tipo ('number'|'boolean'|'string')
    children[0]: Identifier
    children[1] (opcional): expressão inicial
    """
    ...
    def generate(self, st: SymbolTable) -> None:
        ident: Identifier = self.children[0]
        # criar a variável (com shift) também no st de geração
        st.create_variable(ident.value, self.value)
        var = st.get(ident.value)
        Code.append(f"  sub esp, 4 ; var {ident.value} int [EBP-{var.shift}]")
        if len(self.children) == 2:
            self.children[1].generate(st)      # EAX = init
            Code.append(f"  mov [ebp-{var.shift}], eax")
...
class BinOp(Node):
    ...
    def generate(self, st: SymbolTable) -> None:
        ...
        # Avaliar direita, empilhar; depois esquerda em EAX; ECX guarda o da pilha
        # Padrão: right -> push ; left -> eax ; pop ecx  (assim sub/div preservam ordem a-b, a/b)
        ...
        if op == '+':
            ...
            return
        if op == '-':
            Code.append("  sub eax, ecx")  # left - right
            return
        if op == '*':
            Code.append("  imul ecx")      # eax *= ecx
            return
        if op == '/':
            # left / right
            Code.append("  cdq")           # edx:eax
            Code.append("  idiv ecx")
            return
        if op == '%':
            Code.append("  cdq")
            Code.append("  idiv ecx")
            Code.append("  mov eax, edx")  # resto
            return

        # ----- Relacionais num/num -----
        if op in ('<','>','<=','>=','==','!=','===','!=='):
            # comparação em geral: cmp eax(left), ecx(right)
            Code.append("  cmp eax, ecx")
            Code.append("  mov eax, 0")
            Code.append("  mov ecx, 1")

            if op == '<':
                Code.append("  cmovl eax, ecx"); return
            if op == '>':
                Code.append("  cmovg eax, ecx"); return
            if op == '<=':
                Code.append("  cmovle eax, ecx"); return
            if op == '>=':
                Code.append("  cmovge eax, ecx"); return
            if op in ('==','==='):
                Code.append("  cmove eax, ecx"); return
            if op in ('!=','!=='):
                Code.append("  cmovne eax, ecx"); return

        # ----- Booleanos (0/1) -----
        if op == '&&':
            # if left == 0 -> 0; else -> (right != 0)
            lbl_f = f"and_false_{self.id}"
            lbl_e = f"and_end_{self.id}"
            Code.append("  cmp eax, 0")
        ...
        if op == '||':
            ...
            lbl_t = f"or_true_{self.id}"
            lbl_e = f"or_end_{self.id}"
            Code.append("  cmp eax, 0")
